...
/*
 * SMOTE01110
 * Modifications by Nir Ofek. are marked with 'modified'
 * 
 * a. 0: original. the distance of calculating knn is based on nominal and numeric attributes
 * 	  1: the distance calculated is based on numeric attributes only
 * b. 0: original. calculate knn with non normalized values
 * 	  1: calculating the knn based on normalized distance
 * c. 0: original. randomizing the 'gap' to be used for each attribute's value separately
 * 	  1: use same random 'gap' with all attribute
 * d. 0: original. Artificial's nominal values will chosen by the method-distance table
 *    1: Artificial's nominal values will be taken "as is" from the closest instance (current, nearest neighbor) according to randomized 'gap
 * e. 0: Distance is powered by 2, for numeric and nominal
 *    
 */
  ...
  protected void doSMOTE() throws Exception {
    ...
    // modified b
    // holds the minimum and maximum of numeric attributes' values
    Map<Attribute, double[]> minMaxMap = new HashMap<Attribute, double[]>();
    // end modified b
    
    // modified a+b
    // holds the maximum distance from each nominal attributes' values. attribute value represented by its int index
    Map<Attribute, double[]> NomMinMaxMap = new HashMap<Attribute, double[]>();
    // end modified a+b
	...
	if (attr.isNominal() || attr.isString()) {
		  ...
		  // ********** modified a+b.   
	      boolean[] minorityValues = new boolean[attr.numValues()]; // flag if value occurs in any minority instances
		  // ********** ENDmodified a+b.
		  ...
		  while(instanceEnum.hasMoreElements()) {
				...
				// ********** modified a+b.
			    if (classValue==minIndex) // is minority instance
			    	minorityValues[value]=true;
       			// ********** ENDmodified a+b.
		  }
		  for (int valueIndex1 = 0; valueIndex1 < attr.numValues(); valueIndex1++) {
			  //********* modified a+b
		       double[] max = new double[attr.numValues()]; // for each value index, store the maximal distance form any minority instance
		       NomMinMaxMap.put(attr, max );
		       max[valueIndex1]=-1*Double.MAX_VALUE; // will be the maximum distance from the current value
		       //********* ENDmodified a+b
			   ...
			   for (int valueIndex2 = 0; valueIndex2 < attr.numValues(); valueIndex2++) {
				  ...
				  //********* modified a+b		      
			      if (sum>max[valueIndex1] && minorityValues[valueIndex2]==true)
			    	  max[valueIndex1]=Math.abs(sum);
	...
	else // numeric
	{
		// init values
	       ...
	       minMaxMap.put(attr, minMax );
	       minMax[0]=Double.MAX_VALUE; // will be the minimum value
	       minMax[1]=-1*Double.MAX_VALUE;// will be the maximum value
	       instanceEnum = getInputFormat().enumerateInstances();
	       while(instanceEnum.hasMoreElements()) {
			   Instance instance = (Instance) instanceEnum.nextElement();
			   if ((int) instance.classValue() == minIndex) { // is minority instance
			       // get the abs maximum distance form instance
			       double doubleValue =  instance.value(attr);
	...
	if (i != j) {
	  ...
	  while(attrEnum.hasMoreElements()) {
	    ...
	    if (!attr.equals(getInputFormat().classAttribute())) {
	      ...
	      if (attr.isNumeric()) {
	  	    // ***********modified b. get the abs maximum distance form instance
	    	  double maxDist = Math.max(Math.abs( ((double[])minMaxMap.get(attr))[0]-instanceI.value(attr)), Math.abs( ((double[])minMaxMap.get(attr))[1]-instanceI.value(attr)));
	    	if(maxDist!=0)
	    		distance += Math.pow( (iVal - jVal)/maxDist, 2);
	    	else
	    		distance += 0;
	  	    // ***********ENDmodified b
	  	    //original
	    	...
	    	// ***********modified a+b
	    	  if (((double[])NomMinMaxMap.get(attr))[(int)iVal]!=0)
	    		  distance +=Math.pow(  (  ((double[][]) vdmMap.get(attr))[(int) iVal][(int) jVal]  ) / ((double[])NomMinMaxMap.get(attr))[(int)iVal] ,2);
	    	  else
	    		  distance += 0;
	    	// ***********ENDmodified a+b
	    	  //original
	  ...
	  if (!attr.equals(getInputFormat().classAttribute())) {
	    if (attr.isNumeric()) {
	      ...
	      values[attr.index()] = (long) (instanceI.value(attr) + gap * dif);
	    	...
	    	//modified d
	    	/* Nominal: the nominal attribute value assigned is of the current, or selected nearest neighbor
	    	 * depands on rand*/
	    	if (gap>0.5)
	    		values[attr.index()] =nnArray[nn].value(attr);
	    	
	    	else
	    		values[attr.index()] =instanceI.value(attr);
	      // original nominal
	      /*
