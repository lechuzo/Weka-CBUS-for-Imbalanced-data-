...
public class HellingerDistanceAttributeEval
   ...
   * Constructor
   */
  public HellingerDistanceAttributeEval () {
    resetOptions();
  }
  ...
  public void buildEvaluator (Instances data)
    ...
    if (numClasses!=2){
		  throw  new Exception("Hellinger distance han handle only two class values, whereas this data has  " 
                + numClasses + " calss values");
    }
    ...
    // Compute info gains
    m_Hellinger = new double[data.numAttributes()];
    for (int i = 0; i < data.numAttributes(); i++) {
      if (i != classIndex) {
    	  m_Hellinger[i] = getHellinger(counts[i]); 
  ...
  /**
   * calculate Hellinger distance for a single attribute
   * @param matrix
   * @return
   * @throws Exception
   */
  public double getHellinger (double[][] matrix){
	  double Hdist=0, classProp=0;
	  double [] sumClasses = new double[matrix[0].length];
	  if (matrix[0].length!=2){
		  return 0;
	  }
	  
	  // sum instances in each class 
	  for (int j=0;j<matrix.length;j++){ //foreach attribute value
		  for (int i=0;i<matrix[0].length;i++){ //foreach class value
			  sumClasses[i]+= matrix[j][i];
		  }
	  }
	  
	// sum differences of class propotions in each attribute value
	  for (int j=0;j<matrix.length;j++){ 		  
		  for (int i=0;i<matrix[0].length;i++){ //foreach class value
			  if (i==0){
				  classProp= Math.sqrt(matrix[j][i]/sumClasses[i]);
			  }else{ //second class
				  classProp-= Math.sqrt(matrix[j][i]/sumClasses[i]);
			  }
		  }
		  Hdist+=classProp*classProp; // power 2 the difference of class proportions in each attribute value
	  }
	  Hdist=Math.sqrt(Hdist);
  ...
  public static void main (String[] args) {
	  ...
	  runEvaluator(new HellingerDistanceAttributeEval(), args);
  }
}
