 ...
 * @author Nir Ofek
 */
public class FastCbus extends SbcClassifier implements Serializable{
    ...
    public FastCbus() throws Exception{
        super();
    ...
    public void buildClassifier(Instances data) throws Exception {    
        ...
        attsArray[0]=onlyMinority.classIndex();
        remove.setAttributeIndicesArray(attsArray);
        remove.setInputFormat(onlyMinority);
        Instances onlyMinorityNoClass = Filter.useFilter(onlyMinority, remove) ;
        
        this.m_Clusterer.buildClusterer( onlyMinorityNoClass );
        buildModelsForClusters(onlyMinority,onlyMajority);
    ...
    public double[] distributionForInstance(Instance instance) throws Exception {
        ...
        /*******     outside the nearest cluster - set as majority  *****/        
        if (!isInsideCluster(clusterIndex, toClusterInst, distancesFromClusters[clusterIndex])){
            if (printStats==true)
            {
                if(expectedClassValue!=1-minorityClass)
                    outOfClusterFalse++;
                else
                    outOfClusterCorrect++;
                ClassifiedOutCnt++;
                System.out.print(ClassifiedOutCnt+"@");
             }
            double[] results = new double[numClassValues];
            results[this.minorityClass]=1-0.99;
            results[1-this.minorityClass]=1-0.01;
            return results;
        }        
     ...
     * each model will be trained based on: all minority instances closest to the corresponding cluster
     * with probability > thresh. the closest majority class instances will be taken as m ration.  
     * @param data
     * @param thresh
     * @param m
     * @return
     * @throws Exception
     */
    private void buildModelsForClusters(Instances onlyMinority,Instances onlyMajority) throws Exception{
        if (this.m_Classifier == null) {
            throw new Exception("A base classifier has not been specified!");
        }
    	this.classIndex=onlyMinority.classIndex();
        ...
        /******************MINORITY**************************/
        // for each minority instance -  set its corresponding cluster.
        maxDist=new double[numOfClusters]; 
        // for each cluster, iterate instances pertain to it, and find the one that has the greatest distance from the centroid
        double dist;
        ...
        for(Instance inst : onlyMinority){
        	...
        	distancesFromClusters= getDistances_by_ed(inst, onlyMinority, false); // check
        	...
        	dist=distancesFromClusters[clusterIndex]; 
        	if (dist>maxDist[clusterIndex]){
        		maxDist[clusterIndex]=dist;
        	}
        }             
        
    
        /******************Majority**************************/
        // go over all majority instances and allocate some to the relevant learning set (cluster)
        /***************************************************/          
        int cntMinority=0;
        int maxMinority=0;
        boolean [] clustersToCheck=new boolean[numOfClusters]; // whether we are done with taking majority for that cluster or not
        Map[] majorityInClustersMap = new HashMap[numOfClusters]; // holds all majority
        for (int i=0;i<numOfClusters;i++){
            majorityInClustersMap[i]= new HashMap<Integer, Double>();
            clustersToCheck[i]=true;
        	cntMinority+=countersArray[i][0];
        	if(countersArray[i][0]>maxMinority){
        		maxMinority=countersArray[i][0];
        	}
        }
        int M=maxMinority;
        double p=(double)maxMinority/cntMinority;
        double k= 1.644; //for 0.95
        double a=p*p;
        double b=-2*p*M-k*k*p+k*k*p*p;
        double c=M*M;
        double sol1=(-1*b+Math.sqrt(b*b-4*a*c)) / (2*a);
        double sol2=(-1*b-Math.sqrt(b*b-4*a*c)) / (2*a);
        int sampleSize=(int) (0.5+Math.max(sol1, sol2)); // choose max of the two solutions to the polynomial function and round to int
        
        Instance current;
        for(int i=0; i<onlyMajority.size() && i < sampleSize; i++){  // we run at O(minority)
            current=onlyMajority.get(i);
            boolean added = false;
            //clusterIndex=m_Clusterer.clusterInstance(current); // check
            distancesFromClusters= getDistances_by_ed(current, onlyMinority, false); // check
        	clusterIndex=getCloseCluster(distancesFromClusters, false); // since maxDist is not assigned, norm=false
           //if (!isInsideCluster(clusterIndex, currentForClusterer, distancesFromClusters[clusterIndex]) || clustersToCheck[clusterIndex]==false ){
            if (clustersToCheck[clusterIndex]==false ){ // check. can add also isInsideCluster constrain 
        	   for (int j=0; j <numOfClusters; j++){ 
        		   if (numOfClusters!=j){
        			   majorityInClustersMap[j].put(i, 0);
        		   }
        	   }
        	   continue;
            }else{
                if (((double)countersArray[clusterIndex][1])/countersArray[clusterIndex][0]<proportion){ // if did not reach ratio m  between minority and majority
                    trainingSetsArray[clusterIndex].add(current);
                    countersArray[clusterIndex][1]++; 
                    added=true;
                }else{
                	clustersToCheck[clusterIndex]=false;// cluster is full
                	for (int j=0; j <numOfClusters; j++){ 
             		   if (numOfClusters!=j){
             			   majorityInClustersMap[j].put(i, 0);
             		   }
        ...
        //add majority instances for training sets, when cluster has insufficient majority instances
        for(int clusterIndex1=0; clusterIndex1<numOfClusters; clusterIndex1++){
            if ((double)(countersArray[clusterIndex1][1])/countersArray[clusterIndex1][0]>=proportion){
            	continue;
            }
        	int cntMaj2=0;
            HashMap<Integer,Double> curMap;
            curMap=(HashMap<Integer, Double>) majorityInClustersMap[clusterIndex1];
            Iterator it = curMap.entrySet().iterator();
            while (it.hasNext()) {
                Map.Entry<Integer,Double> pair = (Map.Entry<Integer,Double>)it.next();
                if(((double)countersArray[clusterIndex1][1]+cntMaj2)/countersArray[clusterIndex1][0]<proportion){
                    trainingSetsArray[clusterIndex1].add(onlyMajority.get(pair.getKey()));
                    cntMaj2++;
                    //for(int j=0; j<numOfClusters; j++){
                    	// majorityInClustersMap[j].remove(pair);
                    //}
                } else{
                	break;
                }
        ...
        for (int i=0;i<this.m_Classifiers.length;i++){
            this.m_Classifiers[i].buildClassifier(trainingSetsArray[i]);
        }
